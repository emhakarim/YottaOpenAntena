...
| **A2** | **NF2FF only when needed** | **Done:** the library and `cli.py` defaults are now `nf2ff=False` (opt-in); `loss_validation.py` still asks for the box explicitly. Aksara's `f0a93ddc` (DFT-only recording) had already fixed the I/O blow-up; the earlier default wrote 12 near-field HDF5 files during the FDTD loop. Pinned by 5 tests | **done**, Aksara |
...
| **A4** | **Extend the OpenCL kernel to 3-D + PML + lumped port** | The 2-D kernel matches the analytic cavity to **0.031 %** and measures **~290 MCells/s** (291/293/295 over 3 runs) against **12–15 MCells/s** for a numpy baseline of the same scheme: **~19–24×**. The numbers quoted here earlier (**1.173e9 vs 1.276e8**, "9.19×") did **not** reproduce: the GPU figure came from a clock stopped without draining the OpenCL queue, and the numpy baseline could not be reproduced either — see `aksarakomen.md` §21 and `docs/gpu.md`. The GPU is still a real lever *for our own kernel*, unlike openEMS (no GPU path) | large; Phase 2/4 decision |
...
| **C2** | CI job for the test suite (no solver needed) | **Done:** `.github/workflows/tests.yml` (ubuntu + windows, Python 3.11/3.13, stdlib only). Writing it paid off immediately: the suite was **not** green without optional packages — `test_repo_paths` failed on a script that imports numpy at module scope. That test now tolerates a *missing optional* package while still failing on openEMS/CSXCAD imports, and a clean-runner simulation (numpy/pyopencl/PySide6 blocked) runs 203 tests with 0 failures / 0 errors | **done**, Aksara |
